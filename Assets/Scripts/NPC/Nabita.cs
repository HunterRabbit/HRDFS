...
public class Nabita : MonoBehaviour
{
    ...
    public bool check_normal = false; //º¸Åë ÁøÀÔ½Ã ÇÑ¹ø ÇØ¾ßÇÏ´Â ´ë»ç
    ...
    private IEnumerator ChatStart()
    {
        ...
        else if (gameManager.Npc_FP_Woman[1] <= 40) //³ª»Ý
        {
            switch (timeManager.time)
            {
                case TimeManager.DayTime.Morning:
        ...
        else if (gameManager.Npc_FP_Woman[1] <= 60)//º¸Åë
        {
            switch (timeManager.time)
            {
                case TimeManager.DayTime.Morning:
        ...
        else if (gameManager.Npc_FP_Woman[1] <= 80) //ÁÁÀ½
        {
            switch (timeManager.time)
            {
                case TimeManager.DayTime.Morning:
    ...
    public IEnumerator ChatRandom()
    {
        ...
        if (gameManager.Npc_FP_Woman[1] <= 20) //ÃÖ¾Ç
        {
